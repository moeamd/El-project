...
import { logIn, signInWithGoogle, signInWithGithub, resetPassword } from "../features/auth/auth"
        ...
        <main className={' w-full  h-screen bg-[#1B1B1B]/60'}>
            <div className="w-full h-full flex justify-center items-center">
                ...
                <div className="w-full h-full max-w-[800px] md:max-h-[550px] sm:h-auto flex flex-col sm:flex-row shadow-[0_4px_30px_rgba(0,0,0,0.1)] sm:rounded-[8px] ">
                    ...
                    <div className={' relative w-full md:w-1/2 h-full md:h-auto flex flex-col justify-stretch  md:justify-between  bg-[#FFFFFF] p-5 sm:rounded-tr-[8px] sm:rounded-br-[6px]'}>

                        <X size={20} className="cursor-pointer absolute top-2 right-2"
                            onClick={() => { navigate(-2); }} />
                        ...
                        <form className={'flex flex-col gap-3'} onSubmit={formik.handleSubmit}>
                            ...
                            <div className={' flex justify-end my-0'}>
                                <span onClick={() => { resetPassword(formik.values.email) }} className={'text-xs  hover:underline cursor-pointer hover:text-[#3DCBB1]'}>Forget Password?</span>
                            </div>

                            <input type="submit" value="Log In" className={' bg-[#3DCBB1] hover:bg-[#19a88e] w-full text-white font-semibold mt-[25px] md:mt-1 py-[10px] px-[18px] rounded-[14px] items-center border"'} />
                        </form>

                        <div className={'text-center mt-[50px] md:mt-4'}>
                            Need an account? <span onClick={() => { navigate('/signup') }} className={'text-[#3DCBB1] hover:underline cursor-pointer '}>Sign Up</span>
                        </div>
                    </div>
                </div>
            </div >
